...
`short_description` of its own. The number/text-addressed generic tools
...
These tests pin three things:
  1. the three identity tools (search_records / get_record / find_similar)
     refuse task_sla before issuing any request,
  2. `filter_records` still accepts it (the caller names the fields there),
  3. `query_table_by_text` resolves the dot-walked `task.short_description`
     from the table so a text search on task_sla still builds a valid query.
"""
...
class TestTaskSlaTextSearchField:
    """A free-text search on task_sla must dot-walk task.short_description.

    task_sla has no description column of its own, so a bare `short_description`
    condition is silently dropped and returns unrelated rows. The dedicated
    similar_slas_for_text tool was culled in v5.0; the protection now lives in
    `query_table_by_text`, which resolves the search field from the table
    (exercised via TestIntelligentSearchPath below and here).
    """

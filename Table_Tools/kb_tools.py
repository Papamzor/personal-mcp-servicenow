...
async def similar_knowledge_for_text(input_text: str, kb_base: Optional[str] = None, category: Optional[str] = None) -> dict[str, Any] | str:
    ...
    url = f"{NWS_API_BASE}/api/now/table/kb_knowledge?sysparm_fields=number,short_description,kb_knowledge_base,kb_category&sysparm_query={query}"
...
async def get_active_knowledge_articles(input_text: str) -> dict[str, Any] | str:
    ...
    query = "^".join(query_parts)
    url = f"{NWS_API_BASE}/api/now/table/kb_knowledge?sysparm_fields=number,short_description,kb_knowledge_base,kb_category,valid_to&sysparm_query={query}"
    data = await make_nws_request(url)
    if data and data.get('result'):
        return data

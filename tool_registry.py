...
Only fields something reads (plan §3.3). The fourth, `read_only`, arrived when a
consumer did: it is served as the MCP `readOnlyHint` annotation so hosts can
auto-approve the read-only majority. `domain`, `tables`, `public` stay out until a
test or router consumes them.
"""
...
class ToolGuidance:
    """The selection guidance for one tool. All four fields are required.

    `read_only` is served as the MCP `readOnlyHint` tool annotation; writes keep
    the spec's pessimistic defaults (destructive, non-idempotent).
    """
    when_to_use: str
    when_not: str
    prefer_over: str
...
def register_tools(mcp, fns):
    ...
    for fn in fns:
        apply_guidance(fn)
        mcp.tool(annotations={"readOnlyHint": TOOL_GUIDANCE[fn.__name__].read_only})(fn)
    return list(fns)
